...
class painttootl2 {
    ...
    private static void fillArea(char[][] tarray, int startX, int startY) {
        if (startX < 0 || startX >= tarray.length || startY < 0 || startY >= tarray[0].length || tarray[startX][startY] != '.')
            return;

        tarray[startX][startY] = '#';
        fillArea(tarray, startX - 1, startY);
        fillArea(tarray, startX + 1, startY);
        fillArea(tarray, startX, startY - 1);
        fillArea(tarray, startX, startY + 1);
    }
